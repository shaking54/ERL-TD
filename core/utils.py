...
class ReplayBuffer(object):
    ...
    def add(self, data):
        ...
        if len(self.storage) == 110000:
            print("the running time of 10000 frames is {}".format((time.time() - self.start_time)))
    ...
    def sample(self, batch_size):
        self.ind = np.random.randint(0, len(self.storage), size=batch_size)
        x, y, u, r, d,nu, parameters, target_std =[], [],[], [], [], [], [], []
       # print(len(self.storage), "   ", ind)
        
       # print(self.storage[ind[0]])
        
        for i in self.ind:
            X, Y, U, R, D, NU, P, T = self.storage[i]
            ...
            #target_std.append(np.array(T, copy=False))
        return np.array(x), np.array(y), np.array(u), np.array(r).reshape(-1, 1), np.array(d).reshape(-1, 1),np.array(parameters),np.array(nu)

    def update(self, args, target_std):
        history_target_std = []
        for num, i in enumerate(self.ind):
            X, Y, U, R, D, NU, P, T = self.storage[i]
            index = T[0] % args.histroy_std_num
            T[int(index + 1)] = target_std[num]
            T[0] += 1
            self.storage[i] = (X, Y, U, R, D, NU, P, T)
            history_target_std.append(T)
        return history_target_std
