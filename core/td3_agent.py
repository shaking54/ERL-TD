...
class Agent:
    ...
    def evaluate(self, agent: td3.GeneticAgent or td3.TD3, is_render=False, is_action_noise=False, # type: ignore
        ...
        while not done:
            if store_transition:
                self.num_frames += 1
                if not isinstance(agent, td3.TD3):
                    self.gen_frames += 1
    ...
    def train(self):
        ...
        if self.args.RL:
            for eval in range(10):
                sac_stats = self.evaluate(self.rl_agent,store_transition=False, is_action_noise=False)
                testr += sac_stats['reward']
            testr /= 10.0
